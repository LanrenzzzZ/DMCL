...
def binning(lower_bound, data, quantity): # 0, (360, 1) 20
    width = round(len(data) / quantity)    # 18
    quantity -= 1
    bins = []
...
class GazeboEnv:
    ...
    def __init__(self, launchfile, height, width, nchannels):
        ...
        self.image = rospy.Subscriber('/d435/color/image_raw', Image, self.image_callback, queue_size=1)
        self.depth_image_msg = rospy.Subscriber('/d435/depth/image_raw', Image, self.dapth_image_callback, queue_size=1)
    ...
    def step(self, act):
        ...
        toGoal = [Dist / 10, (beta2 + np.pi) / (np.pi * 2), act[0], act[1]]
        self._frames.append(self.cv_image)
        return toGoal, self._get_obs(), reward, done, target
    ...
    def reset(self):
        ...
        # human_state = self.set_human_state
        x = 0
        y = 0
        chk = False
        while not chk:
        ...
        while depth_image is None:
            try:
                depth_image = rospy.wait_for_message('/d435/depth/image_raw', Image, timeout=0.5)
            except:
                pass
        ...
        if beta2 < -np.pi:
            ...
            beta2 = np.pi - beta2
        # 极坐标
        toGoal = [Dist / 10, (beta2 + np.pi) / (np.pi * 2), 0.0, 0.0]  # list
        return toGoal, self._get_obs()
